...
### Added
- **Formal Project Documentation (`docs/`)**: Restructured and linearly indexed theoretical, architectural, and analytical documentation (`01_theory_concept.md` through `07_pub_report_general.md`).
- **Academic Results Expansion**: Detailed theoretical proof of `120s` (2 minutes) being the absolute minimum viable scan length utilizing `Marginal Gain` (First Derivative) analysis and `Uncertainty Decay` tracking.
- **Advanced Plotting Module**: Synthesized the 2x2 comprehensive visualization grid demonstrating statistical predictive accuracy, diminishing marginal efficiency, and noise extraction (`Figure1_Combined.png`). 
