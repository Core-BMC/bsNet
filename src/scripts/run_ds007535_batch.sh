#!/usr/bin/env bash
# run_ds007535_batch.sh — ds007535 duration sweep, all atlases
#
# 사용법:
#   cd /path/to/bsNet
#   source .venv/bin/activate
#   bash src/scripts/run_ds007535_batch.sh
#
# 옵션 (환경변수로 오버라이드):
#   N_SEEDS=10        bash src/scripts/run_ds007535_batch.sh
#   N_JOBS=4          bash src/scripts/run_ds007535_batch.sh
#   ATLASES="schaefer200 schaefer400"  bash ...
#   FORCE=1           bash ...  # 기존 결과 덮어쓰기
#
# 기본 동작: 이미 집계 CSV가 있으면 skip (--skip과 동일)
...
echo -e "${CYAN}=== DS007535 Duration Sweep Batch ===${NC}"
