#!/usr/bin/env python3
"""ABIDE duration sweep: BS-NET performance across observation durations.

For each short-scan duration (30s–180s), runs BS-NET on ABIDE PCP subjects
and records r_FC (raw) and ρ̂T (BS-NET) vs full-scan reference FC.

Replaces the synthetic duration sweep (Figure 1) with real-data evidence.

Design:
  - Common subset: only subjects with total scan ≥ min_total_sec (default 360s)
...
Usage:
    # Full run (N≈396, 10 seeds, 6 durations) — ~1 hour
    python src/scripts/run_abide_duration_sweep.py --atlas cc200

    # Quick test
    python src/scripts/run_abide_duration_sweep.py --atlas cc200 --max-subjects 5 --n-seeds 2

    # Parallel
    python src/scripts/run_abide_duration_sweep.py --atlas cc200 --n-jobs 4
"""
...
DURATIONS_SEC = [30, 60, 90, 120, 150, 180]
DEFAULT_SEEDS = list(range(42, 52))  # 10 seeds: 42–51
CORRECTION_METHOD = "fisher_z"
TARGET_DURATION_MIN = 15
N_BOOTSTRAPS = 100

# Site-specific TR (same as run_abide_bsnet.py)
TR_MAP = {
...
def discover_subjects(
    ...
    Args:
        ...
        max_subjects: Limit number of subjects (0 = all).

    Returns:
        List of dicts with sub_id, ts_path, tr, n_vols, total_sec.
    """
    ...
    # Load subject metadata from existing JSON (for site/TR info)
    ...
    logger.info(
        f"Discovered {len(subjects)} subjects with ≥{min_total_sec}s "
        f"(atlas={atlas})"
    )
    return subjects
...
def _worker(args: tuple) -> list[dict]:
    ...
    sub, durations, seed = args
    return sweep_single_subject(sub, durations, seed)
...
def run_duration_sweep(
    ...
    """Run ABIDE duration sweep experiment.

    Args:
        atlas: Atlas name.
        durations: List of short-scan durations (seconds).
        seeds: List of random seeds.
        min_total_sec: Minimum total scan duration for inclusion.
    ...
    subjects = discover_subjects(atlas, min_total_sec, max_subjects)
    if not subjects:
    ...
    logger.info(
        f"Duration sweep: {n_sub} subjects × {n_seeds} seeds × "
        f"{n_dur} durations = {n_sub * n_seeds * n_dur} runs"
    )

    # Build task list: (subject, durations, seed)
    tasks = [
        (sub, durations, seed)
        for sub in subjects
        for seed in seeds
    ...
    csv_path = results_dir / f"abide_duration_sweep_{atlas}.csv"
    fieldnames = [
    ...
    # ── Aggregate: duration × seed → mean/std ────────────────────────────
    agg_path = results_dir / f"abide_duration_sweep_{atlas}_aggregated.csv"
    _save_aggregated(all_results, agg_path, durations, seeds)
    logger.info(f"Aggregated CSV: {agg_path}")

    # ── Print summary ────────────────────────────────────────────────────
    print(f"\n{'='*60}")
    print(f"ABIDE Duration Sweep — {atlas.upper()}")
    ...
    for dur in durations:
        ...
        if not dur_rows:
            continue
        # Average across subjects and seeds
...
def _save_aggregated(
    ...
    """
    # Step 1: per (duration, seed) → subject-averaged metrics
    ...
    # Step 2: per duration → across-seed mean ± SD
    rows = []
    for dur in durations:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="ABIDE duration sweep for BS-NET Figure 1"
    )
    parser.add_argument(
        "--atlas", default="cc200", choices=["cc200", "cc400"],
    )
    parser.add_argument(
        "--durations", nargs="+", type=int, default=DURATIONS_SEC,
        help="Short-scan durations in seconds (default: 30 60 90 120 150 180)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--min-total-sec", type=float, default=360.0,
        help="Minimum total scan duration for subject inclusion (default: 360s)",
    )
    parser.add_argument(
        "--max-subjects", type=int, default=0,
        help="Limit subjects (0 = all, default: 0)",
    )
    parser.add_argument(
